...
class BucketORP:
    ...
    def permute(self):
        print(f"\n[INFO] Permute starting for N = {self.N}, Z = {self.bucket_size}, B = {self.B}")
    ...
    def merge_split_routing(self, buckets):
        ...
        for r in range(rounds):
            ...
            for i in range(0, len(current_buckets), 2):
                ...
                if r == 0:
                    bits_seen = [(elem.aux >> r) & 1 for elem in merged if elem.aux != -1]
                    print(f"  Bit 0 routing: {bits_seen.count(0)} → 0, {bits_seen.count(1)} → 1")
                ...
                if r == rounds - 1:  # Only enforce overflow constraint in the final routing round
